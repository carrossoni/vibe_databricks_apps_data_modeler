...
class DatabricksUnityService:
    ...
    def list_catalogs(self) -> List[Dict[str, Any]]:
        ...
        try:
            catalogs = []
            logger.info("🔍 Starting catalog listing...")
            catalog_list = list(self.client.catalogs.list())
            logger.info(f"📊 Found {len(catalog_list)} catalogs")
            
            for catalog in catalog_list:
                logger.info(f"📁 Processing catalog: {catalog.name}")
                catalogs.append({
                    'name': catalog.name,
            ...
            logger.info(f"✅ Successfully processed {len(catalogs)} catalogs")
            return catalogs
                ...
                if tag_key and tag_key.strip():
                    ...
                    if tag_value != current_value:  # Only set if different
                        if current_value is not None:
                            # Tag exists - UNSET first, then SET
        ...
        for i, change in enumerate(tag_changes):
            try:
                ...
                if action == 'CREATE':
                    ...
                    # For UPDATE: UNSET first, then SET the new value
                    tag_value = change['tag_value']
                    
                    # First UNSET the existing tag
                    if entity_type == 'tables':
                        unset_statement = f"UNSET TAG ON TABLE {entity_id} `{tag_key}`"
                        set_statement = f"SET TAG ON TABLE {entity_id} `{tag_key}` = `{tag_value}`"
                    else:  # columns
                        unset_statement = f"UNSET TAG ON COLUMN {entity_id} `{tag_key}`"
                        set_statement = f"SET TAG ON COLUMN {entity_id} `{tag_key}` = `{tag_value}`"
                    
                    print(f"   🔍 Executing SQL (1/2): {unset_statement}")
                    
                    # Execute UNSET first
                    unset_response = self.client.statement_execution.execute_statement(
                        warehouse_id=warehouse_id,
                        statement=unset_statement,
                        wait_timeout="30s"
                    )
                    
                    if unset_response.status.state != StatementState.SUCCEEDED:
                        print(f"   ❌ UNSET TAG failed: {unset_response.status.state}")
                        if unset_response.status.error:
                            print(f"   ❌ Error: {unset_response.status.error}")
                        continue
                    else:
                        print(f"   ✅ UNSET TAG successful: {tag_key}")
...
REFERENCES {full_table_name}({constraint['ref_field_name']})"""
    ...
    def _get_column_tags_from_information_schema(self, catalog_name: str, schema_name: str, table_name: str) -> dict:
        ...
        try:
            print(f"🔍 QUERYING INFORMATION_SCHEMA.COLUMN_TAGS for {catalog_name}.{schema_name}.{table_name}")
            ...
            print(f"🔍 Tag query statement ID: {statement_response.statement_id}")
            print(f"🔍 Tag query status: {statement_response.status.state}")
            
            # Parse results
            current_tags = {}
            if statement_response.result and statement_response.result.data_array:
                print(f"🔍 Found {len(statement_response.result.data_array)} tag rows")
                        ...
                        if tag_assignments:
                            ...
                            for assignment in tag_assignments:
                                ...
                                if tag_key:
                                    ...
                                    print(f"   📋 Found tag: {column.name}.{tag_key} = '{tag_value}'")
                            
                            if column_tags:
                                current_tags[column.name] = column_tags
                        else:
                            print(f"   📋 No tags found for column {column.name}")
                    elif response.status_code == 404:
                        print(f"   📋 No tags found for column {column.name} (404 - not found)")
                    else:
                    ...
                    if tag_assignments:
                        for assignment in tag_assignments:
                            ...
                            if tag_key:
                                ...
                                print(f"   📋 Found table tag: {tag_key} = '{tag_value}'")
                    else:
                        print(f"   📋 No tags found for table {full_table_name}")
                    
                    print(f"🔍 FINAL TABLE TAGS: {table_tags}")
                    return table_tags
                    
                elif response.status_code == 404:
                    print(f"   📋 No tags found for table {full_table_name} (404 - not found)")
                    return {}
                else:
    ...
    def _convert_table_info_to_data_table_with_constraints(self, table_info: TableInfo, 
        ...
        # Auto-size table based on field count
    ...
    def _parse_metric_view(self, view_name: str, view_definition: str, catalog_name: str, schema_name: str, position_index: int = 0) -> Optional[MetricView]:
        ...
        try:
            ...
            version = yaml_data.get('version', '0.1')
            ...
            for join_data in yaml_data.get('joins', []):
                ...
                if not join_name and 'source' in join_data:
                    # Extract table name from source (e.g., "catalog.schema.table_name" -> "table_name")
                ...
                join = MetricViewJoin(
                    ...
                    sql_on=join_data.get('sql_on', join_data.get('on', '')),
            ...
            # Note: source_table_id will be the full table reference (e.g., "catalog.schema.table_name")
            ...
            if metric_view.joins:
                ...
                for join in metric_view.joins:
                    ...
                    # If no direct sql_on, try to generate from column mappings
                        ...
                        for i in range(min(len(join.left_columns), len(join.right_columns))):
                            ...
                            if left_col and right_col:
                                ...
                                left_alias = 'base'
                                right_alias = join.join_alias if hasattr(join, 'join_alias') and join.join_alias else join.name
                    ...
                    if hasattr(join, 'joined_table_name') and join.joined_table_name:
                        ...
                        # Fix duplicate catalog/schema issue (e.g., catalog.schema.catalog.schema.table)
                    ...
                    # Add the ON condition (use 'on' as the field name for Databricks metric views)
                    if sql_on_condition:
                        ...
                        # Remove catalog/schema prefixes from joined table references (e.g., catalog.table.column -> table.column)
                        ...
                        logger.warning(f"⚠️ No SQL ON condition found for join '{join.name}' - using placeholder")
                        join_data['on'] = f"source.id = {join.name}.id  -- TODO: Configure join condition"
